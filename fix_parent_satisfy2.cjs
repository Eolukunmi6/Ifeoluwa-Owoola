...
const oldHandle = `  const handleSatisfied = async (bookingId: string) => {
    try {
...
const newHandle = `  const handleSatisfied = async (bookingId: string) => {
    try {
      setSatisfactionId(bookingId);
...
const oldUI = `{booking.status === 'confirmed' && new Date(booking.scheduled_at) < today && !booking.parent_marked_satisfied && !booking.parent_issue_reported && (
